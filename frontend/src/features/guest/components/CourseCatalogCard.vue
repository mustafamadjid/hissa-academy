...
<template>
  ...
  >
    <div class="p-3">
      ...
      >
        <div class="absolute -right-10 -top-10 size-36 rounded-full bg-lime-accent/20" />
        <div class="absolute -bottom-12 -left-8 size-36 rounded-full bg-white/10" />
        <BookOpen class="relative size-16 text-white/90" :stroke-width="1.3" />
        <span
    ...
    <div class="flex flex-1 flex-col px-6 pb-5 pt-2">
      ...
      <p class="mb-5 line-clamp-3 flex-1 text-sm leading-6 text-neutral-medium">
        {{ course.description }}
      </p>
      ...
      <div v-if="course.progress_percentage > 0" class="mb-4">
        <div class="mb-2 flex items-center justify-between text-xs font-semibold">
          ...
          <span class="text-primary-green">{{ course.progress_percentage }}%</span>
        </div>
      ...
      <div class="flex items-center justify-between gap-3 border-t border-neutral-low/70 pt-4">
        <span class="inline-flex items-center gap-1.5 text-xs text-neutral-medium">
          <CircleCheck class="size-4 text-primary-green" />
          {{ course.completed_lessons }}/{{ course.total_lessons }} materi
        </span>
        <span class="inline-flex items-center gap-1.5 text-sm font-semibold text-primary-green">
          Pelajari sekarang
