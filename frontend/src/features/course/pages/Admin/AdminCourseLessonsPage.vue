  ...
  <AdminDashboard>
    <main class="mx-auto w-full max-w-350 px-4 py-6 md:px-6 md:py-8">
      ...
      >
        ...
        <div class="space-y-3">
          <USkeleton v-for="index in 3" :key="index" class="h-24 w-full rounded-xl" />
        </div>
      </div>
      ...
      >
        <span class="mb-4 flex size-12 items-center justify-center rounded-full bg-red-50 text-red-600">
          <AlertCircle :size="23" aria-hidden="true" />
        </span>
        <h1 class="text-lg font-bold text-slate-900">Lesson course tidak tersedia</h1>
        <p class="mt-2 max-w-md text-sm leading-6 text-slate-500">
          {{ errorMessage || 'Course tidak valid atau sudah tidak tersedia.' }}
        </p>
        <UButton color="neutral" variant="outline" class="mt-5" @click="retryFetch">
      ...
      <div v-else class="space-y-7">
        <header class="relative overflow-hidden rounded-xl bg-emerald-950 px-5 py-7 text-white shadow-sm sm:px-7">
          <div class="absolute -right-12 -top-20 size-56 rounded-full bg-emerald-700/40" aria-hidden="true" />
          <div class="relative flex flex-wrap items-start justify-between gap-5">
            <div class="flex items-start gap-4">
              <span class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-white/10 text-emerald-100">
              ...
              <div>
                ...
                <p class="mt-2 max-w-2xl text-sm leading-6 text-emerald-100/75">
                  Atur urutan materi, perbarui judul, dan kelola video YouTube setiap lesson.
                </p>
              </div>
            </div>
